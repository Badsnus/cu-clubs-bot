...
func (epq *EventParticipantQuery) QueryUser() *UserQuery {
	query := (&UserClient{config: epq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := epq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (epq *EventParticipantQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (epq *EventParticipantQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (epq *EventParticipantQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (epq *EventParticipantQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (epq *EventParticipantQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (epq *EventParticipantQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := epq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (epq *EventParticipantQuery) WithUser(opts ...func(*UserQuery)) *EventParticipantQuery {
	query := (&UserClient{config: epq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(eventparticipant.FieldDeletedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.EventParticipant.Query().
//		Select(eventparticipant.FieldDeletedAt).
//		Scan(ctx, &v)
...
func (epq *EventParticipantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*EventParticipant, error) {
	...
	if query := epq.withUser; query != nil {
		if err := epq.loadUser(ctx, query, nodes, nil,
			func(n *EventParticipant, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (epq *EventParticipantQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*EventParticipant, init func(*EventParticipant), assign func(*EventParticipant, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*EventParticipant)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (epq *EventParticipantQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := epq.ctx.Fields; len(fields) > 0 {
		...
		if epq.withUser != nil {
			_spec.Node.AddColumnOnce(eventparticipant.FieldUserID)
		}
	}
